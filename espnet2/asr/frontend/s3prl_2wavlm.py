...
class S3prl2WavLMFrontend(AbsFrontend):
    ...
    def __init__(
        ...
        self.upstream, self.featurizer, self.noise_featurizer = self._get_upstream(frontend_conf)
    ...
    def _get_upstream(self, frontend_conf):
        ...
        noise_featurizer = Featurizer(
            upstream=s3prl_upstream,
            feature_selection=feature_selection,
            upstream_device="cpu",
        )
        return s3prl_upstream, s3prl_featurizer, noise_featurizer
    ...
    def forward(
            self, input: torch.Tensor, input_lengths: torch.Tensor, is_noise=False
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        wavs = [wav[: input_lengths[i]] for i, wav in enumerate(input)]
        self.upstream.eval()
        feats = self.upstream(wavs)
        if is_noise:
            feats = self.noise_featurizer(wavs, feats)
        else:
