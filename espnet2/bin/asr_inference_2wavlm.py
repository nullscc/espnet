...
class Speech2Text:
    ...
    @torch.no_grad()
    def __call__(
            self, speech: Union[torch.Tensor, np.ndarray], noise_speech: Union[torch.Tensor, np.ndarray]
    ) -> List[
        Tuple[
            Optional[str],
            List[str],
        ...
        # data: (Nsamples,) -> (1, Nsamples)
        noise_speech = noise_speech.unsqueeze(0).to(getattr(torch, self.dtype))
        # lengths: (1,)
        noise_speech_lengths = noise_speech.new_full([1], dtype=torch.long, fill_value=noise_speech.size(1))
            
        batch = {"speech": speech, "speech_lengths": lengths, "noise_speech": noise_speech, "noise_speech_lengths": noise_speech_lengths}
