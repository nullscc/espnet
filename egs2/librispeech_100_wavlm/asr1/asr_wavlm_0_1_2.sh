...
if ! "${skip_train}"; then
    ...
    if [ ${stage} -le 11 ] && [ ${stop_stage} -ge 11 ]; then
        ...
        if [ "${num_splits_asr}" -gt 1 ]; then
			...
			if "${collect_npy}"; then
				if "${clean_scp}"; then
            		_opts+="--train_data_path_and_name_and_type ${_asr_train_dir}/clean.scp,speech,${_type} "
				else
            		_opts+="--train_data_path_and_name_and_type ${_asr_train_dir}/noise.scp,speech,${_type} "
				fi
			else
            	_opts+="--train_data_path_and_name_and_type ${_asr_train_dir}/${_scp},speech,${_type} "
				if "${clean_scp}"; then
            		_opts+="--train_data_path_and_name_and_type ${_asr_train_dir}/npy_clean.scp,npy_data,npy "
				else
            		_opts+="--train_data_path_and_name_and_type ${_asr_train_dir}/npy_noise.scp,npy_data,npy "
				fi
			fi
    ...
    if [ ${stage} -le 12 ] && [ ${stop_stage} -ge 12 ]; then
        ...
        if "${use_k2}"; then
          ...
          if "${use_streaming}"; then
              ...
              asr_inference_tool="espnet2.bin.asr_inference_0_1_2"
          fi
        fi
