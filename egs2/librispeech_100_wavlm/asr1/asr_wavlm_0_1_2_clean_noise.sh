    ...
    if [ ${stage} -le 11 ] && [ ${stop_stage} -ge 11 ]; then
        ...
        if [ "${num_splits_asr}" -gt 1 ]; then
			...
			if "${collect_npy}"; then
            	...
            	_opts+="--train_data_path_and_name_and_type ${_asr_train_dir}/npy.scp,npy_data,npy "
				_opts+="--train_data_path_and_name_and_type ${_asr_train_dir}/npy_noise.scp,npy_noise_data,npy "
				fi
			fi
